...
def save_final_kernel(k_2, conf):
    ...
    sio.savemat(os.path.join(conf.output_dir_path, '%s_kernel_x3.mat' % conf.img_name), {'Kernel': k_2})
    if conf.X4:
...
def run_zssr(k_2, conf):
    ...
    if conf.do_ZSSR:
        ...
        print('~' * 30 + '\nRunning ZSSR X%d...' % (4 if conf.X4 else 3))
        if conf.X4:
            ...
            sr = ZSSR(conf.input_image_path, scale_factor=3, kernels=[k_2], is_real_img=conf.real_image, noise_scale=conf.noise_scale).run()
